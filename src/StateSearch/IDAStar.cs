...
{
    // Done!
    public sealed class IDAStar<T>  : IStateSearchable<T> where T : ILayout<T>
    {
        ...
        public IDAStar(T start, T goal)
        {
            ...
            cutOff = start.GetHeuristic(start, goal);
        ...
        private bool SearchAStar(PriorityQueue<State<T>> open, PriorityQueue<State<T>> frontier)
        {
            ...
            cutOff += frontier.Peek().Layout.GetHeuristic(frontier.Peek().Layout, goal.Layout);
